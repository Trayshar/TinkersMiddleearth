...
public class FactionSmeltery extends SmelteryBlock {

	public final SmelteryMainFaction faction;
	public final String factionName;

	public FactionSmeltery(SmelteryMainFaction faction, String factionName) {
		super();
		this.faction = faction;
		this.factionName = factionName;
		this.setCreativeTab(CommonProxy.LotRTiCTab);
		this.setBlockName("lotrtc.smelterty_" + factionName);
	}
	
	@Override
	public int getRenderType() {
		return FactionSmelteryRender.renderId;
	}
	
	@Override
	public TileEntity createNewTileEntity(World world, int metadata) {
//		System.out.println("[" + FMLCommonHandler.instance().getEffectiveSide() + "] TEMeta:" + metadata);
		if (metadata == 0) return new FactionSmelteryLogic(faction);
...
//		System.out.println("[" + FMLCommonHandler.instance().getEffectiveSide() + "] Returns MultiServantLogic");
	...
	public String[] getTextureNames() {
		...
		for(int i = 0; i < textureNames.length; i++){
			textureNames[i] = textureNames[i] + "_" + factionName;
		}
		return textureNames;
	}
